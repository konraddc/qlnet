...
TEST_CASE( "InputLayer", "[layer]" ) {
    qlnet::Pattern<float, int> pattern({ 1.f, 2.f, 3.f }, { 6 });
    ...
    node.update();
    REQUIRE( node_output == pattern.output().at(0) );
}
...
TEST_CASE( "Layer", "[layer]" ) {
    ...
    qlnet::HiddenLayer<float, qlnet::TransferFunc<float>::Identity> hidden_layer(2);
...
TEST_CASE( "Layer connect", "[layer]") {
    ...
    input_layer->set_input(pattern.input());
    auto *hidden_layer = new qlnet::HiddenLayer<float, qlnet::TransferFunc<float>::Identity>(2);
    hidden_layer->connect(input_layer->output());
    auto *output_layer = new qlnet::HiddenLayer<float, qlnet::TransferFunc<float>::Identity>(2);
